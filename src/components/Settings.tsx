  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      <motion.div 
        ...
        animate={{ y: 0 }}
        exit={{ y: '100%' }}
        className="relative bg-white/5 backdrop-blur-3xl rounded-t-[2.5rem] md:rounded-[2.5rem] w-full max-w-xl h-[90vh] md:h-auto md:max-h-[85vh] overflow-hidden flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.5)] border-t md:border border-white/10"
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-black text-white uppercase tracking-tight">Nastavení systému</h2>
              <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.3em]">Konfigurace BioTrackeru</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/10 transition-all text-slate-500 hover:text-white active:scale-90">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-hide space-y-6 relative">
          ...
          <div className="space-y-4 relative z-10">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 flex items-center gap-3 px-1">
              <User size={12} className="text-cyan-primary" /> Biometrický profil
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Váha (kg)</label>
                <input 
                  type="number" 
                  value={localSettings.userWeight} 
                  onChange={e => setLocalSettings(prev => ({ ...prev, userWeight: parseInt(e.target.value) }))}
                  className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 text-sm outline-none focus:border-cyan-primary transition-all text-white font-black shadow-inner" 
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Věk</label>
                <input 
                  type="number" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Metabolismus</label>
              <select 
                value={localSettings.userMetabolism} 
                onChange={e => setLocalSettings(prev => ({ ...prev, userMetabolism: e.target.value as any }))}
                className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 text-sm outline-none focus:border-cyan-primary transition-all text-white h-[54px] font-black shadow-inner appearance-none"
              >
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Týdenní rozpočet (Kč)</label>
              <input 
                type="number" 
              ...
              {[
                ...
                <button 
                  ...
                  className={cn(
                    ...
                    item.id === 'theme' 
                      ? (localSettings.theme === 'dark' ? "bg-white/5 border-white/10 text-slate-300" : "bg-white/40 border-black/5 text-slate-600")
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    item.id === 'theme' 
                      ? (localSettings.theme === 'dark' ? "bg-slate-800" : "bg-slate-300")
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex flex-col gap-4 p-4 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2rem] shadow-sm">
              <div className="flex gap-2">
                <input 
                  type="text" 
                  value={newEffectName} 
                  onChange={e => setNewEffectName(e.target.value)}
                  className="flex-1 p-3.5 rounded-xl bg-white/5 border border-white/10 text-xs outline-none focus:border-cyan-primary transition-all text-white font-black shadow-inner" 
                  placeholder="Nový účinek..." 
                />
                <select 
                  value={newEffectType} 
                  onChange={e => setNewEffectType(e.target.value as Valence)}
                  className="p-3.5 rounded-xl bg-white/5 border border-white/10 text-[10px] outline-none focus:border-cyan-primary transition-all text-white font-black shadow-inner appearance-none"
                >
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto scrollbar-hide pr-1">
                {customEffects.length === 0 ? (
                  <div className="text-center py-6 border border-dashed border-white/10 rounded-2xl bg-white/5">
                    <p className="text-[10px] text-slate-600 font-black uppercase tracking-[0.2em]">Žádné vlastní účinky</p>
                  </div>
                ) : (
                  customEffects.map((effect, i) => (
                    <div key={i} className="flex items-center justify-between p-3.5 bg-white/5 rounded-xl border border-white/10 group hover:bg-white/10 transition-all shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-xs font-black text-white uppercase tracking-tight">{effect.name}</span>
                          <span className={cn(
                      ...
                      <button 
                        ...
                        className="p-2 rounded-lg hover:bg-red-500/10 text-slate-600 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100 active:scale-90"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                onClick={onExport}
                className="py-4 px-4 rounded-[2rem] bg-white/5 backdrop-blur-3xl border border-white/10 hover:bg-white/10 flex flex-col items-center justify-center gap-2 transition-all group shadow-sm"
              >
                <div className="p-3 rounded-2xl bg-white/5 border border-white/10 group-hover:scale-110 transition-transform shadow-inner">
                  <Download className="text-slate-400 group-hover:text-cyan-primary transition-colors" size={20} />
                </div>
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Export</span>
              </button>
              <label className="py-4 px-4 rounded-[2rem] bg-white/5 backdrop-blur-3xl border border-white/10 hover:bg-white/10 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all group shadow-sm">
                <div className="p-3 rounded-2xl bg-white/5 border border-white/10 group-hover:scale-110 transition-transform shadow-inner">
                  <Upload className="text-slate-400 group-hover:text-cyan-primary transition-colors" size={20} />
                </div>
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Import</span>
          ...
          <div className="space-y-4 relative z-10 pt-6 border-t border-white/10">
            ...
            <div className="p-6 rounded-[2.5rem] bg-white/5 backdrop-blur-3xl border border-white/10 space-y-6 shadow-sm">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h4 className="text-lg font-black text-white uppercase tracking-tight">BioTracker Pro</h4>
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Verze 2.4.0 "Glass Edition"</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="flex items-center justify-center gap-2.5 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all group border border-white/5">
                  <Github size={14} className="text-slate-500 group-hover:text-white" />
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-200">GitHub</span>
                </button>
                <button className="flex items-center justify-center gap-2.5 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all group border border-white/5">
                  <ExternalLink size={14} className="text-slate-500 group-hover:text-white" />
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-200">Web</span>
                </button>
              </div>

              <div className="flex items-center justify-center gap-2 pt-2">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-600">Made with</span>
                <Heart size={10} className="text-rose-500 fill-rose-500 animate-pulse" />
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-600">for better health</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-white/5 bg-white/5 backdrop-blur-3xl">
          <button 
            onClick={handleSave}
